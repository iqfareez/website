...
Telegram bot API is a RESTful API that allows you to send message (and many other [things](https://core.telegram.org/bots/api#available-types)) to your bot.
...
require a token to identify the bot identity. The token is created when we [created](#setting-up-telegram-bot) the bot using BotFather.
...
Diagram below shown an anatomy of a Telegram bot API request.
...
We create a function to send message to Telegram. The function will take the target person as a parameter. The message will be
sent to the chat ID.
