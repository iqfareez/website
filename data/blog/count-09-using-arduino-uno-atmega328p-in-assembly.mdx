...
layout: PostLayout
canonicalUrl: https://iqfareez.com/blog/count-09-using-arduino-uno-atmega328p-in-assembly
authors: ['default']
---
...
everyone, so the tasks is simplified **to count from 0-9 only, and repeats**.

Code in assembly in awful (sorry), probably we are too exposed to that sweet high level programming languages like Java,
...
In this article, we're going to dive in how the programs works on [ATMega328P](https://www.microchip.com/en-us/product/ATmega328P).
...
Both ICs I've mentioned above accepts 4-bit BCD input, and outputs to 7-segment display.
...
Ok, enough introduction, now back to our objective, which counting from **0 to 9** and display on the 7-Segment display.
...
the counter value bits one-to-one with the PORTs. Since our port doesnt start from `0`, the bits need be shifted. This can be done through the `lsl` instruction.
...
Delay is necessary, or otherwise the program ran to fast and we cannot see the clear output
...
depends on `L1` (or vise versa, I'm not sure). Let's see how is the results.

## Result

<ReactPlayer url="https://i.imgur.com/3tivdkL.mp4" playing={true} loop={true} />
...
Believe it or not, [this code](https://github.com/iqfareez/microp-lab3/blob/973b0a67983af0e33a4b7ecbdeac03df3d5e07ec/Exp2-Task3/Exp2-Task3/main.asm)
was my first attempt to complete this task as I don't have any idea how to do counter. I know, it is awful code to look at it, consisting of 100+ LOC. Probably belongs to [r/programminghorror](https://www.reddit.com/r/programminghorror/) 😆
