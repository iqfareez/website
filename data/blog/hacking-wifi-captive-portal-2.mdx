...
The console shows no error, it shows that we have made some improvements. But, the it is empty without any message showing it successful or not.

So, I figured it by leaving the `Invoke-WebRequest`command alone, the script works. Remove all the code before the `Invoke-WebRequest` as shown below. Also delete the `-WebSession $session` in `Invoke-WebRequest` line.

![ISE delete nonsense](/static/blog/hack-wifi-2/powershell-delete-nonsense.jpg)

The script will look similar to this pattern:
...
![powershell shortcut run](/static/blog/hack-wifi-2/w-ps-shortcut.jpg)
...
Transfer the copied item to your phone. Then, open the app and click on the **Add shortcut** button.

![Http Shorcut new](/static/blog/hack-wifi-2/http-shortcut-app-2.jpg)
...
TODO: letak gambar nnti

After that, click the **Save** button. The shortcut will now appear in the app. Tap once to test if it is working.

![Http shorcut test finish](/static/blog/hack-wifi-2/http-shortcut-finish.gif)

<details>
    <summary>**In case** the wifi login failed, play around with the **advanced settings**</summary>
...
Done. You may want to create a homescreen shortcut for easy access. Next time you want to log in to the Wi-Fi, just tap the shortcut.

## Concluding remarks

You can also set the script to run every time your computer boot, so that you'll have continuous access to the Wi-Fi network.
